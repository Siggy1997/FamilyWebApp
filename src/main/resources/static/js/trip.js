...
(function initPhotoViewer() {
	...
	window.openPhotoViewer = function (startIdx) {
		...
		pvSwiper = new Swiper('#pvSwiper', {
			...
			on: {
				slideChange() {
					...
					loadSlidesAround(this.activeIndex);
				},
				afterInit() {
					updateThumbActive(this.activeIndex);
					loadSlidesAround(this.activeIndex);
		...
		pvEl.ontouchstart = null;
		pvEl.ontouchmove = null;
		pvEl.ontouchend = null;
		...
		pvEl.ontouchmove = (e) => {
			const dx = Math.abs(e.touches[0].clientX - _pvTouchStartX);
			const dy = Math.abs(e.touches[0].clientY - _pvTouchStartY);
			if (dx > 8 || dy > 8) _pvTouchMoved = true;
		};
